...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    if (huart->Instance == USART6) {
        uint8_t nextPos = (rxHead + 1) % RX_BUFFER_SIZE;
        if (nextPos != rxTail) {  // Проверка на переполнение буфера
            rxBuffer[rxHead] = huart->Instance->DR;
            rxHead = nextPos;
        }
        HAL_UART_Receive_IT(&huart6, &rxBuffer[rxHead], 1);  // Перезапуск прерывания
    }
}
